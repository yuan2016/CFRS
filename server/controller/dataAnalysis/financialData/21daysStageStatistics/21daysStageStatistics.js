...
module.exports = {
  ...
  fetchAll (req, res) {
    ...
    func.connPool1(query, [tableName.daysStageStatistics21, params.offset, params.limit], function (err, rs) {
      if (err) {
  ...
  getCount (req, res) {
    ...
    func.connPool1(query, [tableName.daysStageStatistics21], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
          res.json({
  ...
  refreshData (req, res) {
    if (global.daysCount === 0) {
      global.daysCount++
      pro.exec(shell.daysStageStatistics, function (error, stdout, stderr) {
        if (error !== null) {
          console.log('exec error: ' + error)
          console.log(moment(new Date()).format('YYYY-MM-DD HH:mm:ss') + ' 21天分期统计shell脚本执行失败')
          res.json({code: '500'})
          global.daysCount = 0
        } else {
          console.log(moment(new Date()).format('YYYY-MM-DD HH:mm:ss') + ' 21天分期统计shell脚本执行成功')
          res.json({code: '200'})
          global.daysCount = 0
        }
      })
      console.log(moment(new Date()).format('YYYY-MM-DD HH:mm:ss') + ' 21天分期统计开始执行shell脚本')
    } else {
      res.json({code: '400'})
    }
  },
  getExcelData (req, res) {
    ...
    let queries = analysis(params, 'd_date', 'w')
    let query = sql.dataAnalysis.daysStageStatistics21Excel + queries + sql.dataAnalysis.order
    func.connPool1(query, [tableName.daysStageStatistics21], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
