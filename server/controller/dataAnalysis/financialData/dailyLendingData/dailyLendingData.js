...
function formatExcelDataRSP (rows) {
  return rows.map(row => {
    if (row.日期) {
    row.日期 = moment(row.日期).format('YYYY-MM-DD')
  }
...
module.exports = {
  ...
  fetchAll (req, res) {
    ...
    func.connPool1(query, [tableName.dailyLendingData, params.offset, params.limit], function (err, rs) {
      if (err) {
  ...
  refreshData (req, res) {
    if (global.dailyLendingCount === 0) {
      ...
      pro.exec(shell.dailyLendingData, function (error, stdout, stderr) {
        if (error !== null) {
          console.log('exec error: ' + error)
          console.log(moment(new Date()).format('YYYY-MM-DD HH:mm:ss') + ' 每日放款数据(或注册量统计表)shell脚本执行失败')
          res.json({code: '500'})
          console.log("failed")
          global.dailyLendingCount = 0
        } else {
          console.log(moment(new Date()).format('YYYY-MM-DD HH:mm:ss') + ' 每日放款数据(或注册量统计表)shell脚本执行成功')
          res.json({code: '200'})
          global.dailyLendingCount = 0
        }
      })
      console.log(moment(new Date()).format('YYYY-MM-DD HH:mm:ss') + ' 每日放款数据(或注册量统计表)开始执行shell脚本')
    } else {
      res.json({code: '400'})
    }
  },
...
// After instantiation, you can grab the readstream at any time.
  ...
  getExcelDataRSP (req, res) {
    let params = req.query
    let queries = analysis(params, 'd_date', 'w')
    let query = sql.dataAnalysis.registrationStatisticsReportExcel + queries + sql.dataAnalysis.order
    func.connPool1(query, [tableName.dailyLendingData], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
