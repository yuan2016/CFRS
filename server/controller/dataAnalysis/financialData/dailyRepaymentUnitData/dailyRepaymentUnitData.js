...
module.exports = {
  ...
  fetchAll (req, res) {
    ...
    func.connPool1(query, [tableName.dailyRepaymentUnitData, params.offset, params.limit], function (err, rs) {
      if (err) {
  ...
  getCount (req, res) {
    ...
    func.connPool1(query, [tableName.dailyRepaymentUnitData], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
          res.json({
  ...
  refreshData (req, res) {
    if (global.dailyRepaymentUnitCount === 0) {
      ...
      pro.exec(shell.dailyRepaymentUnitData, function (error, stdout, stderr) {
        if (error !== null) {
