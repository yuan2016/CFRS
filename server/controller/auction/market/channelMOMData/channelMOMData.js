...
module.exports = {
  fetchAll (req, res) {
    ...
    let order = params.order || sql.auction.market.channelMOMData.order
    let query = sql.auction.market.channelMOMData.selectAll + queries + order + sql.auction.market.channelMOMData.selectAllBack
    func.connPool1(query, [tableName.channelMOMData, params.offset, params.limit], function (err, rs) {
      if (err) {
  ...
  getCount (req, res) {
    let params = req.body
    let queries = analysis(params, 'd_date', 'w')
    let query = sql.auction.market.channelMOMData.getCount + queries
    func.connPool1(query, [tableName.channelMOMData], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
          res.json({
  ...
  getExcelData (req, res) {
    ...
    let queries = analysis(params, 'd_date', 'w')
    let query = sql.auction.market.channelMOMData.selectAll + queries
    func.connPool1(query, [tableName.channelMOMData], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
