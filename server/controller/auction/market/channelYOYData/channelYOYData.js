...
module.exports = {
  fetchAll (req, res) {
    ...
    let order = params.order || sql.auction.market.channelYOYData.order
    let query = sql.auction.market.channelYOYData.selectAll + queries + order + sql.auction.market.channelYOYData.selectAllBack
    func.connPool1(query, [tableName.channelYOYData, params.offset, params.limit], function (err, rs) {
      if (err) {
  ...
  getCount (req, res) {
    let params = req.body
    let queries = analysis(params, 'd_date', 'w')
    let query = sql.auction.market.channelYOYData.getCount + queries
    func.connPool1(query, [tableName.channelYOYData], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
          res.json({
  ...
  getExcelData (req, res) {
    ...
    let queries = analysis(params, 'd_date', 'w')
    let query = sql.auction.market.channelYOYData.selectAll + queries
    func.connPool1(query, [tableName.channelYOYData], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
