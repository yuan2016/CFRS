...
module.exports = {
  fetchAll (req, res) {
    ...
    let order = params.order || sql.auction.market.promotionChannelStatisticsWeekly.order
    let query = sql.auction.market.promotionChannelStatisticsWeekly.selectAll + queries + order + sql.auction.market.promotionChannelStatisticsWeekly.selectAllBack
    func.connPool1(query, [tableName.promotionChannelStatisticsWeekly, params.offset, params.limit], function (err, rs) {
      if (err) {
  ...
  getCount (req, res) {
    let params = req.body
    let queries = analysis(params, 'd_date', 'w')
    let query = sql.auction.market.promotionChannelStatisticsWeekly.getCount + queries
    func.connPool1(query, [tableName.promotionChannelStatisticsWeekly], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
          res.json({
  ...
  getExcelData (req, res) {
    ...
    let queries = analysis(params, 'd_date', 'w')
    let query = sql.auction.market.promotionChannelStatisticsWeekly.selectAllExcel + queries + sql.auction.market.promotionChannelStatisticsWeekly.order
    func.connPool1(query, [tableName.promotionChannelStatisticsWeekly], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
...
// After instantiation, you can grab the readstream at any time.
  ...
  getExcelDataRSP (req, res) {
    let params = req.query
    let queries = analysis(params, 'd_date', 'w')
    let query = sql.auction.market.promotionChannelStatisticsWeekly.registrationStatisticsReportExcel + queries + sql.auction.market.promotionChannelStatisticsWeekly.order
