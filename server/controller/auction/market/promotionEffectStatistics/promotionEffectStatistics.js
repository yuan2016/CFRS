...
global.promotionEffectStatisticsCount = 0
...
function formatData (rows) {
  return rows.map(row => {
    ...
    if (row.Settlement_price_h) {
      row.Settlement_price_h = formatCurrency(row.Settlement_price_h)
    }
    if (row.channel_consumption_h) {
      row.channel_consumption_h = formatCurrency(row.channel_consumption_h)
    }
...
function formatExcelData (rows) {
  return rows.map(row => {
    ...
    if (row['结算单价-补录(元)']) {
      row['结算单价-补录(元)'] = formatCurrency(row['结算单价-补录(元)'])
    }
    if (row['渠道消耗-补录(元)']) {
      row['渠道消耗-补录(元)'] = formatCurrency(row['渠道消耗-补录(元)'])
    }
    if (row['新用户充值金额(元)']) {
...
module.exports = {
  fetchAll (req, res) {
    let params = req.body
    let queries = analysis(params, 'd_date', 'w', 't')
    let order = params.order || sql.auction.market.promotionEffectStatistics.order
    let query = sql.auction.market.promotionEffectStatistics.selectAll + queries + order + sql.auction.market.promotionEffectStatistics.selectAllBack
    func.connPool1(query, [tableName.promotionEffectStatistics, tableName.promotionEffectStatisticsAdd, params.offset, params.limit], function (err, rs) {
      if (err) {
  ...
  getCount (req, res) {
    let params = req.body
    let queries = analysis(params, 'd_date', 'w', 't')
    let query = sql.auction.market.promotionEffectStatistics.getCount + queries
    func.connPool1(query, [tableName.promotionEffectStatistics, tableName.promotionEffectStatisticsAdd], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
          res.json({
  ...
  refreshData (req, res) {
    if (global.promotionEffectStatisticsCount === 0) {
      global.promotionEffectStatisticsCount++
      pro.exec(shell.promotionEffectStatistics, function (error, stdout, stderr) {
        if (error !== null) {
          console.log('exec error: ' + error)
          console.log(moment(new Date()).format('YYYY-MM-DD HH:mm:ss') + ' 渠道推广效果统计shell脚本执行失败')
          res.json({code: '500'})
          console.log("failed")
          global.promotionEffectStatisticsCount = 0
        } else {
          console.log(moment(new Date()).format('YYYY-MM-DD HH:mm:ss') + ' 渠道推广效果统计shell脚本执行成功')
          res.json({code: '200'})
          global.promotionEffectStatisticsCount = 0
        }
      })
      console.log(moment(new Date()).format('YYYY-MM-DD HH:mm:ss') + ' 渠道推广效果统计开始执行shell脚本')
    } else {
      res.json({code: '400'})
    }
  },
  getExcelData (req, res) {
    ...
    let queries = analysis(params, 'd_date', 'w', 't')
    let query = sql.auction.market.promotionEffectStatistics.selectAllExcel + queries + sql.auction.market.promotionEffectStatistics.order
    func.connPool1(query, [tableName.promotionEffectStatistics, tableName.promotionEffectStatisticsAdd], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
...
// After instantiation, you can grab the readstream at any time.
  ...
  modify (req, res) {
    ...
    let query = sql.auction.market.promotionEffectStatistics.upadte
    func.connPool1(query, [tableName.promotionEffectStatisticsAdd, changeItem(params.Settlement_price_h), changeItem(params.channel_consumption_h), req.body.date, req.body.channel_name], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
          res.json({
