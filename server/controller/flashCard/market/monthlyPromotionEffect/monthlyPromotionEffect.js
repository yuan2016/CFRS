...
module.exports = {
  ...
  fetchAll (req, res) {
    ...
    let order = params.order || sql.flashCard.market.monthlyPromotionEffect.order
    let query = sql.flashCard.market.monthlyPromotionEffect.selectAllFront + queries + order
    func.connPool1(query, [tableName.flashCard.monthlyPromotionEffect, params.offset, params.limit], function (err, rs) {
      if (err) {
  ...
  getCount (req, res) {
    let params = req.body
    let queries = analysis(params, 'mm_month', 'a')
    let query = sql.flashCard.market.monthlyPromotionEffect.getCount + queries
    func.connPool1(query, tableName.flashCard.monthlyPromotionEffect, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
          res.json({
  ...
  getSelectOptions (req, res) {
    func.connPool1(sql.flashCard.market.monthlyPromotionEffect.getSelectOptions, tableName.flashCard.monthlyPromotionEffect, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
      }
      rs = packageRows(rs)
  ...
  getExcelData (req, res) {
    ...
    let queries = analysis(params, 'mm_month', 'a')
    let query = sql.flashCard.market.monthlyPromotionEffect.selectAllExcel + queries
    func.connPool1(query, tableName.flashCard.monthlyPromotionEffect, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
