...
module.exports = {
  ...
  fetchAll (req, res) {
    ...
    let order = params.order || sql.flashCard.market.dailyPromotionEffectSummary.order
    let query = sql.flashCard.market.dailyPromotionEffectSummary.selectAllFront + queries + order
    func.connPool1(query, [tableName.flashCard.dailyPromotionEffectSummary, params.offset, params.limit], function (err, rs) {
      if (err) {
  ...
  getCount (req, res) {
    let params = req.body
    let queries = analysis(params, 'd_date', 'a')
    let query = sql.flashCard.market.dailyPromotionEffectSummary.getCount + queries
    func.connPool1(query, tableName.flashCard.dailyPromotionEffectSummary, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
          res.json({
  ...
  getSelectOptions (req, res) {
    func.connPool1(sql.flashCard.market.dailyPromotionEffectSummary.getSelectOptions, tableName.flashCard.dailyPromotionEffectSummary, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
      }
      rs = packageRows(rs)
  ...
  getExcelData (req, res) {
    ...
    let queries = analysis(params, 'd_date', 'a')
    let query = sql.flashCard.market.dailyPromotionEffectSummary.selectAllExcel + queries
    func.connPool1(query, tableName.flashCard.dailyPromotionEffectSummary, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
