...
module.exports = {
  ...
  fetchAll (req, res) {
    ...
    let order = params.order || sql.flashCard.market.dailyPromotionEffect.order
    let query = sql.flashCard.market.dailyPromotionEffect.selectAllFront + queries + order
    func.connPool1(query, [tableName.flashCard.dailyPromotionEffect, params.offset, params.limit], function (err, rs) {
      if (err) {
  ...
  getCount (req, res) {
    let params = req.body
    let queries = analysis(params, 'd_date', 'a')
    let query = sql.flashCard.market.dailyPromotionEffect.getCount + queries
    func.connPool1(query, tableName.flashCard.dailyPromotionEffect, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
          res.json({
  ...
  getSelectOptions (req, res) {
    func.connPool1(sql.flashCard.market.dailyPromotionEffect.getSelectOptions, tableName.flashCard.dailyPromotionEffect, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
      }
      rs = packageRows(rs)
  ...
  getExcelData (req, res) {
    ...
    let queries = analysis(params, 'd_date', 'a')
    let query = sql.flashCard.market.dailyPromotionEffect.selectAllExcel + queries
    func.connPool1(query, tableName.flashCard.dailyPromotionEffect, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
