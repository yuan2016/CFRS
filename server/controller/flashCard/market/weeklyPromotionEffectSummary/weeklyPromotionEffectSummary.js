...
function handleParams(channelName, date) {
  let queries = ''
  if (channelName && !date) {
    queries = ' and channel_name="' + channelName + '"'
  } else if (date && !channelName) {
    year = moment(date).format('YYYY')
    week = moment(date).format('WW')
    queries = ' and y_year=' + year + ' and w_week=' + week
  } else if(channelName && date) {
    year = moment(date).format('YYYY')
    week = moment(date).format('WW')
    queries = ' and channel_name="' + channelName + '" and y_year=' + year + ' and w_week=' + week
  }
  return queries
}
...
module.exports = {
  ...
  fetchAll (req, res) {
    let params = req.body
    let channelName = params.channel_name[0]
    let date = params.startTime[0]
    let queries = handleParams(channelName, date)
    let order = params.order || sql.flashCard.market.weeklyPromotionEffectSummary.order
    let query = sql.flashCard.market.weeklyPromotionEffectSummary.selectAllFront + queries + order
    func.connPool1(query, [tableName.flashCard.weeklyPromotionEffectSummary, params.offset, params.limit], function (err, rs) {
      if (err) {
  ...
  getCount (req, res) {
    ...
    let date = params.startTime[0]
    let queries = handleParams(channelName, date)
    let query = sql.flashCard.market.weeklyPromotionEffectSummary.getCount + queries
    func.connPool1(query, tableName.flashCard.weeklyPromotionEffectSummary, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
          res.json({
  ...
  getSelectOptions (req, res) {
    func.connPool1(sql.flashCard.market.weeklyPromotionEffectSummary.getSelectOptions, tableName.flashCard.weeklyPromotionEffectSummary, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
      }
      rs = packageRows(rs)
  ...
  getExcelData (req, res) {
    ...
    let channelName = params.channel_name.substring(1, params.channel_name.indexOf(','))
    let date = params.startTime.substring(1, params.startTime.indexOf(','))
    let queries = handleParams(channelName, date)
    let query = sql.flashCard.market.weeklyPromotionEffectSummary.selectAllExcel + queries
    func.connPool1(query, tableName.flashCard.weeklyPromotionEffectSummary, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
