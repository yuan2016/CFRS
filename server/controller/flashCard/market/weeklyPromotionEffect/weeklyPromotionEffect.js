...
module.exports = {
  ...
  fetchAll (req, res) {
    ...
    let order = params.order || sql.flashCard.market.weeklyPromotionEffect.order
    let query = sql.flashCard.market.weeklyPromotionEffect.selectAllFront + queries + order
    func.connPool1(query, [tableName.flashCard.weeklyPromotionEffect, params.offset, params.limit], function (err, rs) {
      if (err) {
  ...
  getCount (req, res) {
    ...
    let date = params.startTime[0]
    let queries = handleParams(channelName, date)
    let query = sql.flashCard.market.weeklyPromotionEffect.getCount + queries
    func.connPool1(query, tableName.flashCard.weeklyPromotionEffect, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
          res.json({
  ...
  getSelectOptions (req, res) {
    func.connPool1(sql.flashCard.market.weeklyPromotionEffect.getSelectOptions, tableName.flashCard.weeklyPromotionEffect, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
      }
      rs = packageRows(rs)
  ...
  getExcelData (req, res) {
    ...
    let queries = handleParams(channelName, date)
    let query = sql.flashCard.market.weeklyPromotionEffect.selectAllExcel + queries
    func.connPool1(query, tableName.flashCard.weeklyPromotionEffect, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
