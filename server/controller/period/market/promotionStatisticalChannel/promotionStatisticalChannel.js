...
module.exports = {
  ...
  fetchAll (req, res) {
    ...
    let order = params.order || sql.period.promotionStatisticalChannel.order
    let query = sql.period.promotionStatisticalChannel.selectAllFront + queries + order + sql.period.promotionStatisticalChannel.selectAllBack
    func.connPool1(query, [tableName.period.promotionStatisticalChannel, params.offset, params.limit], function (err, rs) {
      if (err) {
  ...
  getCount (req, res) {
    let params = req.body
    let queries = analysis(params, 'd_date', 'w')
    let query = sql.period.promotionStatisticalChannel.getCount + queries
    func.connPool1(query, [tableName.period.promotionStatisticalChannel], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
          res.json({
  ...
  refreshData (req, res) {
    if (global.channelCount === 0) {
      global.channelCount++
      pro.exec(shell.promotionStatisticalChannel, function (error, stdout, stderr) {
        if (error !== null) {
          console.log('exec error: ' + error)
          console.log(moment(new Date()).format('YYYY-MM-DD HH:mm:ss') + ' 开心分期推广统计(渠道)shell脚本执行失败')
          res.json({code: '500'})
          global.channelCount = 0
        } else {
          console.log(moment(new Date()).format('YYYY-MM-DD HH:mm:ss') + ' 开心分期推广统计(渠道)shell脚本执行成功')
          res.json({code: '200'})
          global.channelCount = 0
        }
      })
      console.log(moment(new Date()).format('YYYY-MM-DD HH:mm:ss') + ' 开心分期推广统计(渠道)开始执行shell脚本')
    } else {
      res.json({code: '400'})
    }
  },
  getSelectOptions (req, res) {
    func.connPool1(sql.period.promotionStatisticalChannel.getSelectOptions, tableName.period.promotionStatisticalChannel, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
      }
      rs = packageRows(rs)
  ...
  getExcelData (req, res) {
    ...
    let queries = analysis(params, 'd_date', 'w')
    let query = sql.period.promotionStatisticalChannel.selectAllExcel + queries
    func.connPool1(query, [tableName.period.promotionStatisticalChannel], function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
