...
let {analysis, formatCurrency, formatInt, handleProperty, handleTime, combine, handleTimeMS, mosaicName, exactAnalysis, removeEmpty, handlePropertyAnd} = require('../../../utils/utils')
...
module.exports = {
  ...
  fetchAll (req, res) {
    ...
    let order = params.order || sql.promotionManagement.promotionRegionStatistics.order
    let query = sql.promotionManagement.promotionRegionStatistics.selectAllFront + queries + order + sql.promotionManagement.promotionRegionStatistics.selectAllBack
  ...
  getCount (req, res) {
    ...
    let queries = analysis(params, 'd_date', 'w')
    let query = sql.promotionManagement.promotionRegionStatistics.getCount + queries
  ...
  refreshData (req, res) {
    if (global.regionCount === 0) {
      global.regionCount++
      pro.exec(shell.promotionRegionStatistics, function (error, stdout, stderr) {
        if (error !== null) {
  ...
  getExcelData (req, res) {
    ...
    let queries = analysis(params, 'd_date', 'w')
    let query = sql.promotionManagement.promotionRegionStatistics.selectAllExcel + queries + sql.promotionManagement.promotionRegionStatistics.order
    func.connPool1(query, tableName.promotionRegionStatistics, function (err, rs) {
      if (err) {
        console.log('[query] - :' + err)
        if (err.message === 'Query inactivity timeout') {
