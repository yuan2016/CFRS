...
let sqlMap = {
  ...
  userData: {
    select:
      "select product_names,available_table,available_table_zww,available_table_jp,available_table_fq,department,role,user_name,user_sex,user_mobile,user_permission from ?? where user_mobile = ?",
    select2:
      "select product_names,available_table,available_table_zww,available_table_jp,available_table_fq,department,role,user_name,user_sex,user_mobile,user_permission from ?? where user_email = ?",
    update:
  ...
  period: {
    ...
    monthlySettlementData: {
      ...
      selectAllExcel:
        'select D_MONTH as "月份",ADVANCE_REPAYMENT_AMT as "提前还款本金(元)",ADVANCE_REPAYMENT_INTEREST as "提前还款利息(元)",REPAYMENT_AMT as "正常还款本金(元)",REPAYMENT_INTEREST as "正常还款利息(元)",OVERDUE_REPAYMENT_AMT as "逾期还款本金(元)",OVERDUE_REPAYMENT_INTEREST as "逾期还款利息(元)",OVERDUE_LATE_FEE as "逾期滞纳金(元)",RENEWAL_FEE as "续期费(元)",YIMATONG_FEE as "益码通手续费(元)",LQ_RECHARGE as "零钱充值(元)",TOTAL_AMT as "合计(元)",CREATE_TIME as "创建时间" from ?? '
    },
