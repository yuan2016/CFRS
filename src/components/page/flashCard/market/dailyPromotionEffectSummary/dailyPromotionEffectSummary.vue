<template>
  <div class="dailyPromotionEffectSummary" v-loading.body="loading" element-loading-text="拼命加载中">
    <banner></banner>
    <div class="date-filter">
      <!-- <li>
        <span class="managerFront">渠道商名称：</span>
        <el-select v-model.trim="channel_name" filterable clearable size="mini" placeholder="不限"
                   class="dailyPromotionEffectSummarySelect">
          <el-option
      ...
      <li>
        ...
        <el-date-picker v-model.trim="startTime" type="date" size="mini" placeholder="从"
                        class="dailyPromotionEffectSummarySelect" value-format="yyyy-MM-dd"></el-date-picker>
    ...
    <el-table :data="fundData" highlight-current-row border stripe style="width: 100%;overflow: auto;" :height="height" class="dailyPromotionEffectSummary-table" @sort-change="sort">
...
<script type="text/ecmascript-6">
  ...
  import {
    getDailyPromotionEffectSummary,
    getDailyPromotionEffectSummaryCount,
    getDailyPromotionEffectSummarySelect
  } from '../../../../../common/js/api'
  ...
  export default {
    ...
    computed: {
      mosaicLink() {
        return 'api/dailyPromotionEffectSummary/excel?channel_name="' + [this.channel_name, 'SELECT'] + '"&startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"'
      },
      ...mapGetters([
        'permission'
      ])
    ...
    methods: {
      ...
      getData() {
        return getDailyPromotionEffectSummary({
      ...
      getCount() {
        return getDailyPromotionEffectSummaryCount({
          channel_name: [this.channel_name, 'SELECT'],
          startTime: [this.startTime, 'DATE'],
          endTime: [this.endTime, 'DATE']
        })
      ...
      getSelectOptions() {
        getDailyPromotionEffectSummarySelect().then((response) => {
  ...
  .dailyPromotionEffectSummary
    ...
    .date-filter
      li
        .managerText
          width: 165px
        .dailyPromotionEffectSummarySelect
          width:140px
</style>
