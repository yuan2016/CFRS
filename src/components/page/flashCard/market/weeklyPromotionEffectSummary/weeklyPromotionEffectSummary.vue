<template>
  <div class="weeklyPromotionEffect" v-loading.body="loading" element-loading-text="拼命加载中">
    ...
    <el-table :data="fundData" highlight-current-row border stripe style="width: 100%;overflow: auto;" :height="height" class="weeklyPromotionEffect-table" @sort-change="sort">
      <el-table-column property="channel_name" fixed label="渠道名称" min-width="100"></el-table-column>
      <el-table-column property="w_week" sortable="custom" label="周期" min-width="150"></el-table-column>
...
<script type="text/ecmascript-6">
  ...
  import {
    getWeeklyPromotionEffectSummary,
    getWeeklyPromotionEffectSummaryCount,
    getWeeklyPromotionEffectSummarySelect
  } from '../../../../../common/js/api'
  ...
  export default {
    ...
    computed: {
      mosaicLink() {
        return 'api/weeklyPromotionEffectSummary/excel?channel_name="' + [this.channel_name, 'SELECT'] + '"&startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"'
      },
      ...mapGetters([
        'permission'
      ])
    ...
    methods: {
      ...
      getData() {
        return getWeeklyPromotionEffectSummary({
      ...
      getCount() {
        return getWeeklyPromotionEffectSummaryCount({
          channel_name: [this.channel_name, 'SELECT'],
          startTime: [this.startTime, 'DATE'],
          endTime: [this.endTime, 'DATE']
        })
      ...
      getSelectOptions() {
        getWeeklyPromotionEffectSummarySelect().then((response) => {
