<template>
  <div class="channelWOWData" v-loading.body="loading" element-loading-text="拼命加载中"
  >
    <banner></banner>
    ...
    <el-table :data="fundData" highlight-current-row border stripe style="width: 100%;overflow: auto;" :height="height"
              class="channelWOWData-table" @sort-change="sort">
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed: {
      mosaicLink() {
        return 'api/channelWOWData/excel?startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"&channel_name="' + [this.channel_name, 'INPUT'] + '"'
      },
      ...mapGetters([
        'permission'
      ])
