<template>
  <div class="dailyPromotionEffectStatistics" v-loading.body="loading" element-loading-text="拼命加载中"
  >
    <banner></banner>
    ...
    <el-table :data="fundData" highlight-current-row border stripe style="width: 100%;overflow: auto;" :height="height"
              class="dailyPromotionEffectStatistics-table" @sort-change="sort">
...
<script type="text/ecmascript-6">
  ...
  import {
    getDailyPromotionEffectStatistics,
    getDailyPromotionEffectStatisticsCount,
    getDailyPromotionEffectStatisticsRefresh
  } from '../../../../../common/js/api'
  ...
  export default {
    ...
    computed: {
      mosaicLink() {
        return 'api/dailyPromotionEffectStatistics/excel?startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"'
      },
      ...mapGetters([
        'permission'
      ])
    ...
    methods: {
      ...
      getData() {
        return getDailyPromotionEffectStatistics({
          limit: this.limit,
          offset: this.offset,
      ...
      getCount() {
        return getDailyPromotionEffectStatisticsCount({
          startTime: [this.startTime, 'DATE'],
          endTime: [this.endTime, 'DATE']
        })
      },
      ...
      refreshData() {
        ...
        getDailyPromotionEffectStatisticsRefresh().then((response) => {
