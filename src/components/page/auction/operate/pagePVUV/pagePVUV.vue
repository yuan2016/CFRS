<template>
  <div class="pagePVUV" v-loading.body="loading" element-loading-text="拼命加载中"
  >
    <banner></banner>
    <div class="date-filter">
      <li>
        ...
        <el-date-picker v-model.trim="startTime" type="date" size="mini" placeholder="从"
                        class="pagePVUVTimeSelect" value-format="yyyy-MM-dd"></el-date-picker>
    ...
    <el-table :data="fundData" highlight-current-row border stripe style="width: 100%;overflow: auto;" :height="height"
              class="pagePVUV-table" @sort-change="sort">
      <el-table-column property="d_date" sortable="custom" label="日期" min-width="90"></el-table-column>
      <el-table-column prop="url" label="页面地址" min-width="100">
        <template slot-scope="scope"><a :href="scope.row.url">点击查看</a></template>
      </el-table-column>
      <el-table-column property="page_type" label="页面归类"></el-table-column>
      <el-table-column property="uv" label="真实页面UV" min-width="120"></el-table-column>
      <el-table-column property="pv" label="真实页面PV" min-width="120"></el-table-column>
      <el-table-column property="avg_residence_time" label="真实用户人均停留时长" min-width="160"></el-table-column>
...
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    computed: {
      mosaicLink() {
        return 'api/pagePVUV/excel?startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE', 'M'] + '"'
      },
      ...mapGetters([
        'permission'
      ])
