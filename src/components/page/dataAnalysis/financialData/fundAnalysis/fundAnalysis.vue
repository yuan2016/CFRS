<template>
  <div class="fundAnalysis" v-loading.body="loading" element-loading-text="拼命加载中">
    ...
    <el-table :data="fundData" highlight-current-row border stripe style="width: 100%;overflow: auto" :height="height"
              class="fundAnalysis-table" @sort-change="sort">
      <el-table-column property="d_date" fixed sortable="custom" label="日期" min-width="90"></el-table-column>
      <el-table-column property="total_amount" label="当日应还总额(元)" min-width="120"></el-table-column>
      <el-table-column property="actual_repayment_amount" label="实际还款金额(元)" min-width="120"></el-table-column>
      ...
      <el-table-column property="renewal_commission" label="续期手续费收入(元)" min-width="130"></el-table-column>
      <el-table-column property="renewal_ratio" label="续期比例"></el-table-column>
      <el-table-column property="overdue_amount" label="逾期金额(元)" min-width="100"></el-table-column>
      <el-table-column property="overdue_proportion" label="逾期比例"></el-table-column>
      <el-table-column property="overdue_payment_amount" label="逾期还款金额(元)" min-width="120"></el-table-column>
      <el-table-column property="late_fees_income" label="滞纳金收入(元)" min-width="110"></el-table-column>
      <el-table-column property="comp_service_income" label="综合服务费收入(元)" min-width="130"></el-table-column>
      <el-table-column property="service_charge" label="实收服务费(元)" min-width="110"></el-table-column>
      <el-table-column property="equal_amount_income" label="同等金额收益(元)" min-width="120"></el-table-column>
      <el-table-column property="capital_surplus" label="当日资金盈余(元)" min-width="120"></el-table-column>
...
<script type="text/ecmascript-6">
  ...
  import {
    getFundAnalysis,
    getFundAnalysisCount,
    getFundAnalysisRefresh
  } from '../../../../../common/js/api'
  ...
  export default {
    ...
    computed: {
      mosaicLink() {
        return 'api/fundAnalysis/excel?startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"'
      },
      ...mapGetters([
        'permission'
      ])
    ...
    methods: {
      ...
      refreshData() {
        ...
        getFundAnalysisRefresh().then((response) => {
          if (response.data.code === '200') {
            this.getDataInit()
            this.buttonLoading = false
            this.$message({
              message: '资金分析刷新完毕，请查看',
              type: 'success'
            })
            ...
            setTimeout(() => {
              this.buttonLoading = false
              this.$message.error('资金分析一键刷新出现错误，请检查网络或联系管理员')
            }, 1000)
          }
        }).catch(() => {
          this.buttonLoading = false
