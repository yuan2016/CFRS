<template>
  <div class="daysStageStatistics" v-loading.body="loading" element-loading-text="拼命加载中" >
    <banner></banner>
    ...
    <el-table :data="fundData" highlight-current-row border stripe class="daysStageStatistics-table"
      ...
      <el-table-column property="d_date" fixed label="到期日" sortable="custom" width="100"></el-table-column>
...
<script type="text/ecmascript-6">
  ...
  import {
    getDaysStageStatistics,
    getDaysStageStatisticsCount,
    getDaysStageStatisticsRefresh
  } from '../../../../../common/js/api'
  ...
  export default {
    ...
    computed: {
      mosaicLink () {
        return 'api/daysStageStatistics21/excel?startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"'
      },
      ...mapGetters([
        'permission'
      ])
    ...
    methods: {
      ...
      getCount () {
        return getDaysStageStatisticsCount({
          startTime: [this.startTime, 'DATE'],
          endTime: [this.endTime, 'DATE']
        })
      },
      ...
      refreshData () {
        ...
        getDaysStageStatisticsRefresh().then((response) => {
