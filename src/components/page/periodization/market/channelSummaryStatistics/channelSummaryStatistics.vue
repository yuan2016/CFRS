<template>
  <div class="channelSummaryStatistics" v-loading.body="loading" element-loading-text="拼命加载中"
  >
    <banner></banner>
    ...
    <el-table :data="fundData" highlight-current-row border stripe style="width: 100%;overflow: auto;" :height="height"
              class="channelSummaryStatistics-table" @sort-change="sort">
...
<script type="text/ecmascript-6">
  ...
  import {
    getChannelSummaryStatistics,
    getChannelSummaryStatisticsCount,
    getChannelSummaryStatisticsRefresh
  } from '../../../../../common/js/api'
  ...
  export default {
    ...
    computed: {
      mosaicLink() {
        return 'api/channelSummaryStatistics/excel?startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"'
      },
      ...mapGetters([
        'permission'
      ])
    ...
    methods: {
      ...
      getData() {
        return getChannelSummaryStatistics({
          limit: this.limit,
          offset: this.offset,
      ...
      getCount() {
        return getChannelSummaryStatisticsCount({
          startTime: [this.startTime, 'DATE'],
          endTime: [this.endTime, 'DATE']
        })
      },
      ...
      refreshData() {
        ...
        getChannelSummaryStatisticsRefresh().then((response) => {
