<template>
  <div class="userRechargeIntervalAnalysis" v-loading.body="loading" element-loading-text="拼命加载中"
  >
    <banner></banner>
    ...
    <el-table :data="fundData" highlight-current-row border stripe style="width: 100%;overflow: auto;" :height="height"
              class="userRechargeIntervalAnalysis-table" @sort-change="sort">
      ...
      <el-table-column property="avg_days7_recharge" sortable="custom" label="7日内平均充值次数" min-width="150"></el-table-column>
...
<script type="text/ecmascript-6">
  ...
  import {
    getUserRechargeIntervalAnalysisQE,
    getUserRechargeIntervalAnalysisQECount
  } from '../../../../../common/js/api'
  ...
  export default {
    ...
    computed: {
      mosaicLink() {
          return 'api/userRechargeIntervalAnalysisQE/excel?startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"'
      },
      ...mapGetters([
        'permission'
      ])
    ...
    methods: {
      ...
      getData() {
        return getUserRechargeIntervalAnalysisQE({
          limit: this.limit,
          offset: this.offset,
      ...
      getCount() {
        return getUserRechargeIntervalAnalysisQECount({
          startTime: [this.startTime, 'DATE'],
          endTime: [this.endTime, 'DATE']
        })
      },
