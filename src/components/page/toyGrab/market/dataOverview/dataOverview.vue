<template>
  <div class="dateOverview" v-loading.body="loading" element-loading-text="拼命加载中">
    ...
    <ul class="date-filter">
      <li>
        <el-select v-model="value" size="mini" placeholder="日" @change="changeValue" class="dateOverviewSelect">
          <el-option
      ...
      <li class="timeHorizon">
        ...
        <el-date-picker v-model.trim="d_date" type="date" size="mini" placeholder="日期"
                        class="d_date" value-format="yyyy-MM-dd" @change="search"></el-date-picker>
      </li>
      <li>
    ...
    <el-card class="box-card">
      <div class="cards">
        <div v-for="(item,key) in dayData" class="card" :key="key">
    ...
    <el-table stripe :data="fundData" highlight-current-row border
      ...
      <el-table-column property="success_num" label="成功抓取次数(正常+折扣+免费)" min-width="190"></el-table-column>
      <el-table-column property="game_factincome"  sortable="custom" label="游戏币实际收入(游戏币)" min-width="180"></el-table-column>
      <el-table-column property="game_theoryincome" sortable="custom" label="游戏币理论收入(游戏币)" width="180"></el-table-column>
      <el-table-column property="income_diff" sortable="custom" label="收入差(游戏币)" min-width="130"></el-table-column>
...
<script type="text/ecmascript-6">
  ...
  import {getDataOverviewRoom, getDataOverviewRoomCount, getDataOverviewDay, getDataOverviewRefresh} from '../../../../../common/js/api'
  ...
  export default {
    ...
    methods: {
      ...
      getCount() {
        return getDataOverviewRoomCount({
          d_date: [this.d_date, 'DATE', 'E']
        })
      },
      search() {
      ...
      refreshData() {
        ...
        getDataOverviewRefresh().then((response) => {
      ...
      setHeight() {
        ...
        this.height = docH - filterH - bannerH - pageH - 105 - 142
      },
      sort(info) {
