<template>
  <div class="bannerPVUV" v-loading.body="loading" element-loading-text="拼命加载中"
  >
    <banner></banner>
    ...
    <el-table :data="fundData" highlight-current-row border stripe style="width: 100%;overflow: auto;" :height="height"
              class="bannerPVUV-table" @sort-change="sort">
      ...
      <el-table-column property="banner_name" label="banner图名称" min-width="120"></el-table-column>
      <el-table-column property="pv" label="各个banner图点击量" min-width="140"></el-table-column>
      <el-table-column property="uv" label="各个banner图用户数" min-width="140"></el-table-column>
      <el-table-column property="banner_times" label="各banner详情停留时间" min-width="150"></el-table-column>
    ...
    computed: {
      mosaicLink() {
        if (this.startTime === this.endTime || this.startTime === formatDate(new Date(), 'yyyy-MM-dd') || (!this.startTime && !this.endTime)) {
          return 'api/bannerPVUV/excel?startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"'
        } else {
          return 'api/bannerPVUV/excelSUM?startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"'
        }
      },
      ...mapGetters([
        'permission'
