<template>
  <div class="arbitraryGatePVUV" v-loading.body="loading" element-loading-text="拼命加载中"
  >
    <banner></banner>
    ...
    <el-table :data="fundData" highlight-current-row border stripe style="width: 100%;overflow: auto;" :height="height"
              class="arbitraryGatePVUV-table" @sort-change="sort">
      <el-table-column property="d_date" sortable="custom" label="日期" min-width="160"></el-table-column>
      <el-table-column property="pv" label="任意门点击量" min-width="100"></el-table-column>
      <el-table-column property="uv" label="任意门用户数" min-width="100"></el-table-column>
      <el-table-column property="discount_amount" label="折扣次数(5折/免费)赠送量" min-width="160"></el-table-column>
      <el-table-column property="discount_using" label="折扣次数(5折/免费)使用量" min-width="160"></el-table-column>
...
<script type="text/ecmascript-6">
  ...
  import {
    getArbitraryGatePVUV,
    getArbitraryGatePVUVCount,
    getArbitraryGatePVUVSUM
  } from '../../../../../common/js/api'
  ...
  export default {
    ...
    computed: {
      mosaicLink() {
        if (this.startTime === this.endTime || this.startTime === formatDate(new Date(), 'yyyy-MM-dd') || (!this.startTime && !this.endTime)) {
          return 'api/arbitraryGatePVUV/excel?startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"'
        } else {
          return 'api/arbitraryGatePVUV/excelSUM?startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"'
        }
      },
      ...mapGetters([
        'permission'
    ...
    methods: {
      ...
      getDataSUM() {
        return getArbitraryGatePVUVSUM({
          limit: this.limit,
          offset: this.offset,
          startTime: [this.startTime, 'DATE'],
          endTime: [this.endTime, 'DATE'],
      ...
      getCount() {
        return getArbitraryGatePVUVCount({
          startTime: [this.startTime, 'DATE'],
          endTime: [this.endTime, 'DATE']
        })
      },
