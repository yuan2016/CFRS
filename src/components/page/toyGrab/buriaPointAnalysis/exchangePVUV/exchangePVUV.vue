<template>
  <div class="exchangePVUV" v-loading.body="loading" element-loading-text="拼命加载中"
  >
    <banner></banner>
    ...
    <el-table :data="fundData" highlight-current-row border stripe style="width: 100%;overflow: auto;" :height="height"
              class="exchangePVUV-table" @sort-change="sort">
      <el-table-column property="d_date" sortable="custom" label="日期" min-width="160"></el-table-column>
      <el-table-column property="product_name" label="各兑换产品名称" min-width="110"></el-table-column>
      <el-table-column property="pv" label="各兑换按钮点击量" min-width="120"></el-table-column>
      <el-table-column property="uv" label="各兑换按钮用户量" min-width="120"></el-table-column>
      <el-table-column property="exchange_times" label="各个兑换详情页停留时间" min-width="160" ></el-table-column>
    ...
    computed: {
      mosaicLink() {
        if (this.startTime === this.endTime || this.startTime === formatDate(new Date(), 'yyyy-MM-dd') || (!this.startTime && !this.endTime)) {
          return 'api/exchangePVUV/excel?startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"'
        } else {
          return 'api/exchangePVUV/excelSUM?startTime="' + [this.startTime, 'DATE'] + '"&endTime="' + [this.endTime, 'DATE'] + '"'
        }
      },
      ...mapGetters([
        'permission'
