...
<script>
  ...
  import { getWeeklyPassRateAndOverdueTrends } from '../../../../common/js/api'
  ...
  export default {
    ...
    methods: {
      getDataInit () {
//        this.axios.post('/api/weeklypassRateAndOverdueTrends/weekly', {
...
//          })
//        })
        getWeeklyPassRateAndOverdueTrends({channelName: this.channelName || '汇总'}).then((res) => {
          if (res.data) {
            this.data = res.data
      ...
      initData () {
        this.chart = echarts.init(document.getElementById('weeklypassRateAndOverdueTrends-pic'))
        const option = {
          //标题
          title: {
            text: '通过率/逾期率周趋势',
