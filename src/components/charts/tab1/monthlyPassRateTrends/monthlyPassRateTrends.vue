<template>
  <div class="monthlyPassRateTrends">
    <div id="monthlyPassRateTrends-pic" style="width: 100%;height: 100%;"></div>
  </div>
</template>
...
<script>
  ...
  import { getMonthlyPassRateTrends } from '../../../../common/js/api'
  ...
  export default {
    ...
    methods: {
      getDataInit () {
        getMonthlyPassRateTrends().then((res) => {
          if (res.data) {
            this.data = res.data
      ...
      initData () {
        this.chart = echarts.init(document.getElementById('monthlyPassRateTrends-pic'))
        const option = {
          //标题
          title: {
            text: '通过率月趋势',
